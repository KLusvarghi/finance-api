...
import { makeCreateUserController, makeDeleteUserController, makeGetUserBalanceController, makeGetUserByIdController, makeUpdateUserController, } from '@/factories/controllers/user';
...
app.patch('/api/transactions/:transactionId', async (request, response) => {
    ...
    const { statusCode, body } = await updateTransactionsController.execute(request);
    response.status(statusCode).send(body);
});
app.delete('/api/transactions/:transactionId', async (request, response) => {
    ...
    const { statusCode, body } = await deleteTransactionsController.execute(request);
    response.status(statusCode).send(body);
});
app.listen(port, () => {
