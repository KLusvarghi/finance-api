import { createTestTransaction, createTestUser, updateTransactionParams as params, } from '@/test';
...
describe('PostgresUpdateTransactionRepository', () => {
    ...
    describe('error handling', () => {
        it('should throw an error if Prisma throws', async () => {
            // arrange
            jest.spyOn(prisma.transaction, 'update').mockRejectedValueOnce(new Error('Prisma error'));
            // act
    ...
    describe('success', () => {
        it('should update a transaction on database successfully', async () => {
            ...
            expect(dayjs(response?.date).daysInMonth()).toBe(dayjs(transaction.date).daysInMonth());
            expect(dayjs(response?.date).month()).toBe(dayjs(transaction.date).month());
