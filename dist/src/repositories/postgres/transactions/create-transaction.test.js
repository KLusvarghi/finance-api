...
import { createTestUser, createTransactionParams, transactionId, createTransactionRepositoryResponse as fakeTransaction, } from '@/test';
...
describe('PostgresCreateTransactionRepository', () => {
    ...
    describe('error handling', () => {
        it('should throw an error if Prisma throws', async () => {
            // arrange
            jest.spyOn(prisma.transaction, 'create').mockRejectedValueOnce(new Error('Prisma error'));
            // act
    ...
    describe('success', () => {
        it('should create a transaction on database', async () => {
            ...
            expect(String(response.amount)).toBe(String(createTransactionParams.amount));
            expect(dayjs(response.date).daysInMonth()).toBe(dayjs(createTransactionParams.date).daysInMonth());
            expect(dayjs(response.date).month()).toBe(dayjs(createTransactionParams.date).month());
            expect(dayjs(response.date).year()).toBe(dayjs(createTransactionParams.date).year());
        });
    });
