...
describe('PostgresGetTransactionsByUserIdRepository', () => {
    ...
    describe('error handling', () => {
        it('should throw an error if Prisma throws', async () => {
            // arrange
            jest.spyOn(prisma.transaction, 'findMany').mockRejectedValueOnce(new Error('Prisma error'));
            // act
    ...
    describe('success', () => {
        it('should get transactions by user id on database', async () => {
            ...
            expect(String(response?.[0].amount)).toBe(String(transaction.amount));
            expect(dayjs(response?.[0].date).daysInMonth()).toBe(dayjs(transaction.date).daysInMonth());
            expect(dayjs(response?.[0].date).month()).toBe(dayjs(transaction.date).month());
            expect(dayjs(response?.[0].date).year()).toBe(dayjs(transaction.date).year());
        });
    });
