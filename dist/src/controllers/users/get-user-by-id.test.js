...
import { userId, getUserByIdServiceResponse, getUserByIdHttpRequest as baseHttpRequest, invalidUUID, } from '@/test';
describe('GetUserByIdController', () => {
    ...
    describe('error handling', () => {
        it('should return 404 if user is not found', async () => {
            jest.spyOn(getUserByIdService, 'execute').mockRejectedValue(new UserNotFoundError(userId));
            const result = await sut.execute(baseHttpRequest);
            expect(result.statusCode).toBe(404);
            expect(result.body?.status).toBe('error');
            expect(result.body?.message).toBeTruthy();
            expect(result.body?.message).toContain(userId);
            expect(result.body?.message).toBe(`User with id ${userId} not found`);
        });
