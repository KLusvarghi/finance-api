...
import { userId, updateUserParams, updateUserRepositoryResponse, updateUserHttpRequest as baseHttpRequest, invalidUUID, } from '@/test';
describe('UpdateUserController', () => {
    ...
    describe('error handling', () => {
        ...
        it('should return 400 when UpdateUserService throws EmailAlreadyExistsError', async () => {
            jest.spyOn(updateUserService, 'execute').mockRejectedValueOnce(new EmailAlreadyExistsError(baseHttpRequest.body.email));
            const result = await sut.execute(baseHttpRequest);
        ...
        it('should return 404 when UpdateUserService throws UserNotFoundError', async () => {
            jest.spyOn(updateUserService, 'execute').mockRejectedValueOnce(new UserNotFoundError(baseHttpRequest.params.userId));
            const result = await sut.execute(baseHttpRequest);
            expect(result.statusCode).toBe(404);
            expect(result.body?.status).toBe('error');
            expect(result.body?.message).toBeTruthy();
            expect(result.body?.message).toContain(baseHttpRequest.params.userId);
        });
        it('should return 500 when UpdateUserService throws UpdateUserFailedError', async () => {
            jest.spyOn(updateUserService, 'execute').mockRejectedValueOnce(new UpdateUserFailedError());
            const result = await sut.execute(baseHttpRequest);
            expect(result.statusCode).toBe(500);
            expect(result.body?.status).toBe('error');
            expect(result.body?.message).toBeTruthy();
    ...
    describe('success cases', () => {
        ...
        it('should call UpdateUserService with correct parameters', async () => {
            ...
            // assert
            expect(spy).toHaveBeenCalledWith(baseHttpRequest.params.userId, baseHttpRequest.body);
