...
import { userId, getUserBalanceServiceResponse, getUserByIdRepositoryResponse, } from '@/test';
describe('GetUserBalanceService', () => {
    let sut;
    let getUserByIdRepository;
    let getUserBalanceRepository;
    ...
    const makeSut = () => {
        ...
        const sut = new GetUserBalanceService(getUserByIdRepository, getUserBalanceRepository);
        return {
            sut,
    ...
    beforeEach(() => {
        const { sut: service, getUserByIdRepository: getUserByIdRepositoryStub, getUserBalanceRepository: getUserBalanceRepositoryStub, } = makeSut();
        sut = service;
    ...
    describe('error handling', () => {
        ...
        it('should throw if GetUserBalanceRepository throws', async () => {
            // arrange
            jest.spyOn(getUserBalanceRepository, 'execute').mockRejectedValueOnce(new Error());
            // act
            const promise = sut.execute(userId);
            // assert
            expect(promise).rejects.toThrow();
