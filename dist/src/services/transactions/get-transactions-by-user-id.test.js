...
import { getTransactionByUserIdServiceResponse, getTransactionByUserIdRepositoryResponse, getUserByIdRepositoryResponse, userId, } from '@/test';
describe('GetTransactionsByUserIdService', () => {
    ...
    const makeSut = () => {
        ...
        const sut = new GetTransactionsByUserIdService(getUserByIdRepository, getTransactionByUserIdRepository);
        return {
            sut,
    ...
    beforeEach(() => {
        const { sut: service, getUserByIdRepository: getUserByIdRepositoryStub, getTransactionByUserIdRepository: getTransactionByUserIdRepositoryStub, } = makeSut();
        sut = service;
    ...
    describe('error handling', () => {
        ...
        it('should throw if GetUserByIdRepository throws', async () => {
            // arrange
            jest.spyOn(getUserByIdRepository, 'execute').mockRejectedValueOnce(new Error('GetUserByIdRepository error'));
            // act
            const promise = sut.execute(userId);
            // assert
            expect(promise).rejects.toThrow(new Error('GetUserByIdRepository error'));
        });
        it('should throw if GetTransactionsByUserIdRepository throws', async () => {
            // arrange
            jest.spyOn(getTransactionByUserIdRepository, 'execute').mockRejectedValueOnce(new Error('GetTransactionsByUserIdRepository error'));
            // act
            const promise = sut.execute(userId);
            // assert
            expect(promise).rejects.toThrow(new Error('GetTransactionsByUserIdRepository error'));
        });
    });
    describe('success', () => {
    ...
    describe('validations', () => {
        ...
        it('should call GetTransactionByUserIdRepository with correct params', async () => {
            // arrange
            const getTransactionByUserIdRepositorySpy = jest.spyOn(getTransactionByUserIdRepository, 'execute');
            // act
            await sut.execute(userId);
            // assert
